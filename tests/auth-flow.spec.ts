...
test.beforeAll(async () => {
  ...
  const profileRes = await api.post(`${SUPABASE_URL}/rest/v1/profiles`, {
    headers: {
      apikey:         SUPABASE_SERVICE_KEY,
      Authorization:  `Bearer ${SUPABASE_SERVICE_KEY}`,
      "Content-Type": "application/json",
  ...
  if (!profileRes.ok()) {
    ...
    console.warn(`[beforeAll] profile upsert: ${profileRes.status()} ${body}`);
  }
});
...
test("TC3: /plans でお試しボタンをクリックするとダッシュボードに遷移する", async ({ page }) => {
  ...
  // alert() が出た場合はキャプチャして自動 dismiss
  page.on("dialog", async dialog => {
    console.log("[TC3 DIALOG]", dialog.message());
    await dialog.dismiss();
  });

  // API レスポンスをキャプチャ（デプロイ確認 + エラー検出）
  page.on("response", res => {
    if (res.url().includes("/api/plans/activate-free")) {
      console.log("[TC3 API]", res.status(), res.url());
    }
  });

  await page.getByRole("button", { name: /お試し（無料・月3枚）で始める/ }).click();

  // API 応答後に DB の実際のplan値を確認（デバッグ）
  await page.waitForTimeout(1000);
  const verifyCtx = await request.newContext();
  const verifyRes = await verifyCtx.get(
    `${SUPABASE_URL}/rest/v1/profiles?id=eq.${testUserId}&select=id,plan`,
    {
      headers: {
        apikey:        SUPABASE_SERVICE_KEY,
        Authorization: `Bearer ${SUPABASE_SERVICE_KEY}`,
  ...
  console.log("[TC3 PROFILE]", JSON.stringify(profileData));
